...
class data_loader():
    ...
    def shuffle_and_split(self,split_ratio):
        ...
        is_train = [data['Graph1'].iloc[i] in train_set and data['Graph2'].iloc[i] in train_set for i in range(len(data))] #and data['Graph1'].iloc[i] != data['Graph2'].iloc[i] 
        is_val = [data['Graph1'].iloc[i] in val_set and data['Graph2'].iloc[i] in val_set for i in range(len(data))] # and data['Graph1'].iloc[i] != data['Graph2'].iloc[i]
        is_test = [data['Graph1'].iloc[i] in test_set and data['Graph2'].iloc[i] in test_set for i in range(len(data))] #and data['Graph1'].iloc[i] != data['Graph2'].iloc[i]
...
def set_to_dict(pair,fold,dataset):
    ...
    for i in range(len(pair)):
        ...
        if "MAO" in dataset:
            ...
            pair_dict["C_wasserstein1"] = fold['C_wasserstein'][str(key)]
            pair_dict["C_gromov1"] = fold['C_gromov'][str(key)]
            g2 = p['Graph2'].split('.')[0]
            key = int(g2[-2:])
            for k,v in fold['nom'].items():
                if v.split('.')[0] == key :
        ...
        pair_dict["C_wasserstein2"] = fold['C_wasserstein'][str(key)]
        pair_dict["C_gromov2"] = fold['C_gromov'][str(key)]
...
def split_dic(data,key_list):
    ...
    wasserstein = {key: data['C_wasserstein'][key] for key in data['C_wasserstein'].keys() if int(key) in key_list}
    gromov = {key: data['C_gromov'][key] for key in data['C_gromov'].keys() if int(key) in key_list}
    out_dic['nom']=nom
    out_dic['nodes']=nodes
    out_dic['edges']=edges
    out_dic['e_label']=e_label
    out_dic['C_wasserstein']=wasserstein
    out_dic['C_gromov']=gromov
