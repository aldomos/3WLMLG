...
def calculate_sigmoid_loss(prediction, data):
    ...
    score = (prediction-target)**2 #!!! à modifier pour ppgn
    return score
...
def calculate_normalized_ged(data):
    ...
    """
    norm_ged = data["target"]/(0.5*(len(data["labels_1"])+len(data["labels_2"])))
    return norm_ged
...
class Metric():
    def __init__(self, instances, ged_scaling=None, min_ged=None, max_ged=None):
        ...
        for i,entry in enumerate(instances):
            ...
            self.ged.append(entry['target'].numpy());
            if ged_scaling is None:
                norm_ged = entry["target"]/(0.5*(len(entry["labels_1"])+len(entry["labels_2"])));
                #self.normalized_ged.append(torch.FloatTensor(np.array(norm_ged)));
                self.normalized_ged.append(torch.from_numpy(np.array(entry["target"])).float())
                #self.normalized_ged.append(0.0)
    ...
    def precision_with_ties(self, y_true_changes, y_score, k=10):
        ...
        relevant_set = set(range(y_true_changes[0]+1));
    ...
    def mse(self, predictions, unnormalized=False):
        if unnormalized:
            predictions = predictions * self.normalization_constant;
            se_list = (predictions - self.ged)**2;
            #se_list = (predictions - 0.0)**2
            return np.mean(se_list);
        else:
            #se_list = (predictions - self.normalized_ged)**2;
            se_list = (predictions - self.ged)**2
            #se_list = (predictions - 0.0)**2
            #print(se_list,self.normalized_ged)
            #se_list = list([float(x) for x in se_list]);
            #norm_pred = predictions * self.normalization_constant
            #print('pred : ', norm_pred[0:10])
            print('GED : ',self.ged[0:10])
            #print('norm_GED : ',self.normalized_ged[0:10])
            
            #print(self.normalization_constant)
            print('output : ',predictions[0:10])
            norm_mse = np.mean(se_list)
            #unorm_mse = np.mean((norm_pred-self.ged)**2)
            #print("validation norm MSE : {:.05f}, Validation Unorm MSE : {:.05f}".format(norm_mse, unorm_mse))
            print("validation norm MSE : {:.05f}".format(norm_mse))
            #print('norm GED : ',self.normalized_ged[0:10])
            return norm_mse
    ...
    def mae(self, predictions, unnormalized=False):
        if unnormalized:
            ...
            #ae_list = np.absolute(predictions - 0.0)
            return np.mean(ae_list);
        else:
            ae_list = np.absolute(predictions - self.ged);
            #ae_list = np.absolute(predictions - 0.0)
            ae_list = list([float(x) for x in ae_list]);
            return np.mean(ae_list);
    ...
    def spearman(self, predictions, mode='macro', unnormalized=False):
        if unnormalized:
            ...
            if mode=="macro":
                ...
                correlation_list = [stats.spearmanr(np.array(self.per_node_dict_normalized_ged_sorted[i],dtype='float32'), \
                                                       np.array(predictions[self.per_node_dict_normalized_ged_sorted_ids[i]],dtype = 'float32'))[0] \
