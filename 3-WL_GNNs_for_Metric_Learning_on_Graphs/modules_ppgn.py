...
class RegularBlock(nn.Module):
    ...
    def __init__(self, config, in_features, out_features):
        ...
        #self.norm = torch.nn.LayerNorm(out_features,eps = 1e-5)
...
class MlpBlock(nn.Module):
    ...
    def __init__(self, in_features, out_features, depth_of_mlp, activation_fn=nn.functional.relu):
        ...
        for i in range(depth_of_mlp):
            self.convs.append(nn.Conv2d(in_features, out_features, kernel_size=1, padding=0, bias=False))
...
class SkipConnection(nn.Module):
    ...
    def __init__(self, in_features, out_features):
        super().__init__()
        self.conv = nn.Conv2d(in_features, out_features, kernel_size=1, padding=0, bias=False)
